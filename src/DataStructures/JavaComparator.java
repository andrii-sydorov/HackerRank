...
public class JavaComparator {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < turns; i++) {
            ...
            ls.add(new Player(data[0], Integer.valueOf(data[1])));
        }
        sc.close();
        Comparator<Player> cmpScore = (x, y) -> x.getScore() > y.getScore() ? 1 : x.getScore() < y.getScore() ? -1 : 0;
        Comparator<Player> cmpName = (x, y) -> x.getName().compareTo(y.getName());
        ls.sort(Comparator.comparing(Player::getScore).reversed().thenComparing(Player::getName));
