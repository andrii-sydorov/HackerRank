 ...
 * and
 * finds n^p . If either n or p is negative, then the method must throw an
 * exception
 * which says "should not be negative.". Also, if both and are zero, then the
 ...
 * and p. The locked stub code in the editor reads the input and sends the values to
 ...
 * and p should be non-negative". If both n and p are zero, the output contains "n and p should not be zero.". This is printed
 ...
 * In the last two cases, at least one out of n and p is negative. So, the exception, "n or p should not be negative.", is
 * printed for these two cases.
 */

public class CalculatorException {

    static Scanner in = new Scanner(System.in);
    static Calculator my_calculator = new Calculator();
...
class Calculator {
    int power(int n, int p) throws Exception {
        ...
        String message = null;
        if (n < 0 || p < 0) {
            message = "n or p should not be negative.";
        } else if (n == 0 && p == 0) {
            message = "n and p should not be zero.";
        }
        if (message != null) {
